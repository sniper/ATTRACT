...
RESOURCE_DIR(resourceDir) {
    ...
    // Create the camera for the scene: MOVED TO BELOW BULLET
    ...
    camera = make_shared<Camera>(GRID_SIZE, bullet);
...
void GameManager::updateGame(double dt) {
    ...
    //step the bullet, update camera
    if (bullet->getStepFlag()) {
        camera->setPosition(bullet->stepAndPrint(dt));
    }
    /* shitty camera collision right here: only y <= 1*/

    if (camera->getPosition().y <= 1) {

        bullet->setStepFlag(false);

        camera->setInputLockFlag(false);
    }
